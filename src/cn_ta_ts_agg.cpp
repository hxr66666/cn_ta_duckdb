...
struct CnTaTsAggP4 {
	...
	static void Update(Vector inputs[], AggregateInputData &, idx_t input_count, Vector &states_vec, idx_t count) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			if (hdata.validity.RowIsValid(hidx) && ldata.validity.RowIsValid(lidx) && cdata.validity.RowIsValid(cidx) &&
			    vdata.validity.RowIsValid(vidx) && tdata.validity.RowIsValid(tidx)) {
				timestamp_t cur_ts = tss[tidx];
...
struct CnTaTsAggP6 {
	...
	static void Update(Vector inputs[], AggregateInputData &, idx_t input_count, Vector &states_vec, idx_t count) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			if (hdata.validity.RowIsValid(hidx) && ldata.validity.RowIsValid(lidx) && tdata.validity.RowIsValid(tidx)) {
				timestamp_t cur_ts = tss[tidx];
...
void RegisterCnTaTsAggFunctions(ExtensionLoader &loader) {
	...
	loader.RegisterFunction(ScalarFunction("cn_ta_clear_holidays", {}, LogicalType::BIGINT, ClearHolidaysFunc));
...
#define TALIB_TS_AGG_P1_DOUBLE(sql_name, ta_func, ta_lookback)                                                         \
	{                                                                                                                  \
		...
		set.AddFunction(AggregateFunction({LogicalType::TIMESTAMP, LogicalType::DOUBLE, LogicalType::INTEGER},         \
		                                  LogicalType::DOUBLE, OP::StateSize, OP::Initialize, OP::Update, OP::Combine, \
		                                  OP::Finalize, FunctionNullHandling::SPECIAL_HANDLING, OP::SimpleUpdate,      \
		                                  nullptr, OP::Destroy));                                                      \
		set.AddFunction(AggregateFunction(                                                                             \
		    ...
		    FunctionNullHandling::SPECIAL_HANDLING, OP::SimpleUpdate, nullptr, OP::Destroy));                          \
...
#define TALIB_TS_AGG_P3_DOUBLE(sql_name, ta_func)                                                                      \
	{                                                                                                                  \
		...
		AggregateFunctionSet set("cta_" #sql_name "_ts");                                                              \
		set.AddFunction(AggregateFunction({LogicalType::TIMESTAMP, LogicalType::DOUBLE, LogicalType::DOUBLE,           \
