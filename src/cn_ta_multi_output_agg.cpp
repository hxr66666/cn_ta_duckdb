...
struct CnTaAggMinMax {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc = TA_MINMAX(0, size - 1, state->values->data(), state->period, &outBeg, &outNb, outMin.data(),
			                          outMax.data());
...
struct CnTaAggAroon {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc = TA_AROON(0, size - 1, state->high->data(), state->low->data(), state->period, &outBeg,
...
struct CnTaAggHtPhasor {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc = TA_HT_PHASOR(0, size - 1, state->values->data(), &outBeg, &outNb, outInPhase.data(),
			                             outQuadrature.data());
...
struct CnTaAggHtSine {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc =
			    TA_HT_SINE(0, size - 1, state->values->data(), &outBeg, &outNb, outSine.data(), outLeadSine.data());
...
struct CnTaAggMama {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc = TA_MAMA(0, size - 1, state->values->data(), state->fast_limit, state->slow_limit, &outBeg,
...
struct CnTaAggMacd {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc =
			    TA_MACD(0, size - 1, state->values->data(), state->fast_period, state->slow_period,
			            state->signal_period, &outBeg, &outNb, outMACD.data(), outSignal.data(), outHist.data());
...
struct CnTaAggBbands {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc = TA_BBANDS(0, size - 1, state->values->data(), state->time_period, state->nb_dev_up,
			                          state->nb_dev_dn, (TA_MAType)state->ma_type, &outBeg, &outNb, outUpper.data(),
...
struct CnTaAggStoch {
	...
	static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
		...
		for (idx_t i = 0; i < count; i++) {
			...
			TA_RetCode rc =
			    TA_STOCH(0, size - 1, state->high->data(), state->low->data(), state->close->data(),
			             state->fastk_period, state->slowk_period, (TA_MAType)state->slowk_matype, state->slowd_period,
			             (TA_MAType)state->slowd_matype, &outBeg, &outNb, outSlowK.data(), outSlowD.data());
...
void RegisterCnTaMultiOutputAggFunctions(ExtensionLoader &loader) {
	// ta_minmax: (DOUBLE, INT) -> STRUCT(min, max)
	{
		using OP = CnTaAggMinMax;
		AggregateFunction func("cta_minmax", {LogicalType::DOUBLE, LogicalType::INTEGER},
		                       MakeStruct({{"min", LogicalType::DOUBLE}, {"max", LogicalType::DOUBLE}}), OP::StateSize,
	...
	// ta_aroon: (DOUBLE, DOUBLE, INT) -> STRUCT(aroon_down, aroon_up)
	{
		using OP = CnTaAggAroon;
		AggregateFunction func("cta_aroon", {LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::INTEGER},
	...
	// ta_ht_phasor: (DOUBLE) -> STRUCT(inphase, quadrature)
	{
		using OP = CnTaAggHtPhasor;
		AggregateFunction func("cta_ht_phasor", {LogicalType::DOUBLE},
	...
	// ta_ht_sine: (DOUBLE) -> STRUCT(sine, leadsine)
	{
		using OP = CnTaAggHtSine;
		AggregateFunction func("cta_ht_sine", {LogicalType::DOUBLE},
	...
	// ta_mama: (DOUBLE, DOUBLE, DOUBLE) -> STRUCT(mama, fama)
	{
		using OP = CnTaAggMama;
		AggregateFunction func("cta_mama", {LogicalType::DOUBLE, LogicalType::DOUBLE, LogicalType::DOUBLE},
	...
	// ta_macd: (DOUBLE, INT, INT, INT) -> STRUCT(macd, signal, hist)
	{
		using OP = CnTaAggMacd;
		AggregateFunction func(
		    "cta_macd", {LogicalType::DOUBLE, LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER},
	...
	// ta_stoch: (DOUBLE x3, INT x5) -> STRUCT(slowk, slowd)
	{
		using OP = CnTaAggStoch;
		AggregateFunction func("cta_stoch",
