...
struct P1Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], int, int *, int *, double[]);
	int period;
};
static const std::vector<P1Def> P1_INDICATORS = {
    ...
    {"rocp", TA_ROCP, 10},
    {"rocr", TA_ROCR, 10},
    {"rocr100", TA_ROCR100, 10},
    {"trix", TA_TRIX, 12},
    {"linearreg", TA_LINEARREG, 14},
    {"linearreg_slope", TA_LINEARREG_SLOPE, 14},
    {"linearreg_intercept", TA_LINEARREG_INTERCEPT, 14},
    {"linearreg_angle", TA_LINEARREG_ANGLE, 14},
    {"tsf", TA_TSF, 14},
    {"sum", TA_SUM, 10},
    {"max", TA_MAX, 10},
    {"min", TA_MIN, 10},
...
struct P3Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], const double[], const double[], int, int *, int *, double[]);
	int period;
};
static const std::vector<P3Def> P3_INDICATORS = {
    {"atr", TA_ATR, 14}, {"natr", TA_NATR, 14},       {"willr", TA_WILLR, 14},
    {"cci", TA_CCI, 14}, {"adx", TA_ADX, 14},         {"adxr", TA_ADXR, 14},
    {"dx", TA_DX, 14},   {"plus_di", TA_PLUS_DI, 14}, {"minus_di", TA_MINUS_DI, 14},
};

// P4：high/low/close/volume（无 period）
struct P4Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], const double[], const double[], const double[], int *, int *,
	                   double[]);
};
...
struct P6Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], const double[], int *, int *, double[]);
};
...
struct P7Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], const double[], const double[], int *, int *, double[]);
};
...
struct P8Def {
	const char *name;
	TA_RetCode (*func)(int, int, const double[], const double[], int, int *, int *, double[]);
	int period;
};
static const std::vector<P8Def> P8_INDICATORS = {
    {"midprice", TA_MIDPRICE, 14},
    {"plus_dm", TA_PLUS_DM, 14},
    {"minus_dm", TA_MINUS_DM, 14},
};
...
static void StatRegress(const std::vector<double> &y, const std::vector<double> &x, double &slope, double &intercept,
                        double &r2, double &std_err) {
...
static void ComputeAll(CnTaIndicatorsGlobalState &g) {
	...
	for (auto &ind : P3_INDICATORS) {
		...
		vector<double> outReal(n);
		TA_RetCode rc =
		    ind.func(0, n - 1, high.data(), low.data(), close.data(), ind.period, &outBeg, &outNb, outReal.data());
		vector<double> col;
		ExpandResult(col, outReal.data(), outBeg, outNb, n);
		add_col(string(ind.name) + "_" + std::to_string(ind.period), std::move(col));
	}
	...
	for (auto &ind : P4_INDICATORS) {
		...
		vector<double> outReal(n);
		TA_RetCode rc =
		    ind.func(0, n - 1, high.data(), low.data(), close.data(), volume.data(), &outBeg, &outNb, outReal.data());
		vector<double> col;
		ExpandResult(col, outReal.data(), outBeg, outNb, n);
	...
	for (auto &ind : P5_INDICATORS) {
		...
		vector<double> outReal(n);
		TA_RetCode rc =
		    ind.func(0, n - 1, open.data(), high.data(), low.data(), close.data(), &outBeg, &outNb, outReal.data());
		vector<double> col;
		ExpandResult(col, outReal.data(), outBeg, outNb, n);
		add_col(ind.name, std::move(col));
	}
	...
	auto expand3 = [&](const string &n1, const string &n2, const string &n3, const double *o1, const double *o2,
	                   const double *o3, int outBeg, int outNb) {
	...
	// BBANDS（布林带）：(close, 20, 2.0, 2.0, SMA)
	{
		...
		TA_BBANDS(0, n - 1, close.data(), 20, 2.0, 2.0, TA_MAType_SMA, &outBeg, &outNb, up.data(), mid.data(),
		          lo.data());
	...
	// STOCH（随机指标）：(HLC, 5, 3, SMA, 3, SMA)
	{
		...
		TA_STOCH(0, n - 1, high.data(), low.data(), close.data(), 5, 3, TA_MAType_SMA, 3, TA_MAType_SMA, &outBeg,
		         &outNb, k.data(), d.data());
...
static unique_ptr<FunctionData> CnTaIndicatorsBind(ClientContext &context, TableFunctionBindInput &input,
	...
	for (auto &ind : P1_INDICATORS)
		add_out(string(ind.name) + "_" + std::to_string(ind.period));
	for (auto &ind : P3_INDICATORS)
		add_out(string(ind.name) + "_" + std::to_string(ind.period));
	for (auto &ind : P4_INDICATORS)
		add_out(ind.name);
	...
	// 多输出指标列
	for (const char *c : {"bbands_upper", "bbands_middle", "bbands_lower", "macd", "macd_signal", "macd_hist",
	                      "stoch_k", "stoch_d", "aroon_down", "aroon_up", "minmax_min", "minmax_max", "mama", "fama",
	...
	// Eigen 金融统计（整段常量列）
	for (const char *c : {"stat_var", "stat_stddev", "stat_skew", "stat_kurtosis", "stat_max_drawdown", "stat_sharpe",
	                      "stat_annual_vol", "stat_corr_close_vol", "stat_beta", "stat_regress_slope",
	                      "stat_regress_intercept", "stat_regress_r2", "stat_regress_std_err", "stat_trend_strength"}) {
		add_out(c);
	}

	// 滚动统计（每行列，窗口 20）
	for (const char *c : {"roll_avg_20", "roll_sum_20", "roll_min_20", "roll_max_20", "roll_momentum_20", "roll_var_20",
	                      "roll_stddev_20"}) {
		add_out(c);
	}
