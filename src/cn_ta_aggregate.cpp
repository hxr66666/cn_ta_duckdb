...
struct CnTaAggP1Double {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState1); }
...
struct CnTaAggP1Int {
    ...
    static void Finalize(Vector &states_vec, AggregateInputData &, Vector &result, idx_t count, idx_t offset) {
        ...
        for (idx_t i = 0; i < count; i++) {
            ...
            TA_RetCode rc = TA_FUNC(start, size - 1, state->values->data(), state->period, &outBeg, &outNb, outInt.data());
...
struct CnTaAggP3 {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState3); }
...
struct CnTaAggP4 {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState4); }
...
struct CnTaAggP5Double {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState5); }
...
struct CnTaAggP6 {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState6); }
...
struct CnTaAggP7 {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState7); }
...
struct CnTaAggP8 {
    static idx_t StateSize(const AggregateFunction &) { return sizeof(CnTaAggState8); }
