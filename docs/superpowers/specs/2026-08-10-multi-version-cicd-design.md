# Multi-version CI/CD for the `cn_ta` DuckDB extension
...
Make `cn_ta` build and release cleanly across multiple DuckDB versions on
...
- C++ DuckDB extension named `cn_ta`, wrapping TA-Lib v0.6.4 (CMake `FetchContent`).
...
1. **Signing / distribution:** Community Extensions is the primary signed path
   (`INSTALL cn_ta FROM community; LOAD cn_ta;`, no flags). Also keep self-hosted
...
- `.github/workflows/distribution.yml` calls
  ...
  with `duckdb_version`, `ci_tools_version`, `extension_name: cn_ta`, and
...
- End users: `INSTALL cn_ta FROM community; LOAD cn_ta;` — no flags.

### Self-hosted GitHub Pages (custom repo)

- `.github/workflows/deploy.yml` publishes into a **shared `gh-pages` branch**
  under `/<duckdb_version>/<platform>/cn_ta.duckdb_extension.gz`, writing only its
...
- Fixes two current bugs:
  ...
  - DuckDB's custom-repo layout requires **gzipped** `.duckdb_extension.gz` files
    for `INSTALL cn_ta FROM 'https://neuesql.github.io/cn_ta'` to resolve.
...
- `gh-pages` serves each version under its own subdir; a real
  `INSTALL cn_ta FROM 'https://neuesql.github.io/cn_ta'` resolves for at least
  one version.
- `INSTALL cn_ta FROM community` works once the community PR merges.
