...
Add a new file `src/cn_ta_multi_output_agg.cpp` containing:
...
5. **Registration** - Each function is registered individually (not via x-macro) since they have unique signatures. Register in a new `RegisterCnTaMultiOutputAggFunctions(ExtensionLoader &loader)` called from `cn_ta_extension.cpp`.
...
| File | Change |
|------|--------|
| `src/cn_ta_multi_output_agg.cpp` | **New file** - all 8 window aggregate implementations |
| `src/cn_ta_extension.cpp` | Add `RegisterCnTaMultiOutputAggFunctions` call |
