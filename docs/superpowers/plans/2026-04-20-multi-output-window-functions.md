...
**Architecture:** New file `src/cn_ta_multi_output_agg.cpp` with per-function state structs and aggregate operators that finalize into STRUCT vectors. Each function is registered individually since signatures vary.
...
| File | Role |
|------|------|
| `src/cn_ta_multi_output_agg.cpp` | **Create** - All 8 window aggregate implementations + registration |
| `src/cn_ta_extension.cpp` | **Modify** - Add forward declaration + call to new registration function |
...
**Files:**
- Create: `src/cn_ta_multi_output_agg.cpp`
- Modify: `src/cn_ta_extension.cpp`
...
Add `src/cn_ta_multi_output_agg.cpp` to the `EXTENSION_SOURCES` list in `CMakeLists.txt`, after the existing `src/cn_ta_multi_output.cpp` line:
...
- [ ] **Step 3: Wire up in cn_ta_extension.cpp**

Add forward declaration and call. In `src/cn_ta_extension.cpp`, add after line 14:
...
And in `LoadInternal`, add after `RegisterCnTaMultiOutputFunctions(loader);`:
...
Run: `cd /Users/qunfei/Projects/cn_ta && make`
...
```bash
git add src/cn_ta_multi_output_agg.cpp src/cn_ta_extension.cpp CMakeLists.txt
...
**Files:**
- Modify: `src/cn_ta_multi_output_agg.cpp`
...
In `src/cn_ta_multi_output_agg.cpp`, add between the `MakeStruct` helper and the registration function:
...
In `RegisterCnTaMultiOutputAggFunctions`, add:
...
Run: `cd /Users/qunfei/Projects/cn_ta && make && make test`
...
```bash
git add src/cn_ta_multi_output_agg.cpp test/sql/test_multi_output_agg.test
...
In `src/cn_ta_multi_output_agg.cpp`, add a new state struct (after `MultiAggState1`):
...
In `src/cn_ta_multi_output_agg.cpp`, add (after `MultiAggStateHL`):
